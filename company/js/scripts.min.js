...
function slowScroll(s){$("html, body").animate({scrollTop:$(s).offset().top},500)}function scrollProducts(){$(".products .row, .products-sidebar").toggleClass("done")}$(function(){$(window).scroll(function(){var s=$(this).scrollTop();s>$(this).height()-350?$(".scroll-btn").addClass("active"):$(".scroll-btn").removeClass("active"),$(".main-img").css({transform:"translate(0%, "+s/20+"%)"})}),$(".slide").waypoint({handler:function(s,l){$('.topline-nav-item[data-slide="'+this.element.id+'"]').addClass("active").siblings().removeClass("active")},offset:"30%"}),$(window).scroll(function(){$(window).scrollTop()<100&&$('.topline-nav-item[data-slide="slide1"]').addClass("active").next().removeClass("active")}),$("#call-form").submit(function(){var s=$(this);return $.ajax({type:"POST",url:"./mail.php",data:s.serialize()}).done(function(){$(".call-form").addClass("blur"),$(".call-success").css("display","flex"),setTimeout(function(){s.trigger("reset")},3e3)}),!1})}),$(window).on("load",function(){$(".preloader").delay(1e3).fadeOut("slow")});